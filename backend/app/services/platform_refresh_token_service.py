...
async def cleanup_expired_tokens(db: AsyncSession) -> None:
    ...
    await db.execute(
        delete(PlatformRefreshToken).where(PlatformRefreshToken.expires_at < now)
    )
...
async def revoke_token(db: AsyncSession, token: str) -> bool:
    ...
    result = await db.execute(
        select(PlatformRefreshToken).where(
            PlatformRefreshToken.token_hash == hash_token(token)
        )
    )
...
async def revoke_family(db: AsyncSession, family_id: uuid.UUID) -> None:
    ...
    await db.execute(
        ...
        .where(
            PlatformRefreshToken.family_id == family_id,
...
async def rotate_refresh_token(
    ...
    *,
    ...
    """Validate and rotate a platform refresh token.

    Returns the new PlatformRefreshToken row on success.
    ...
    result = await db.execute(
        select(PlatformRefreshToken).where(
            PlatformRefreshToken.token_hash == hash_token(presented_token)
        )
    )
    record = result.scalar_one_or_none()
    if record is None:
