...
async def get_insights(
    ...
    """Generate AI insights from current asset data."""
    ...
    expiring_result = await db.execute(
        select(func.count(Asset.id)).where(
            and_(base_filter, Asset.warranty_expiry.isnot(None),
    ...
    repair_result = await db.execute(
        select(func.count(AuditLog.id)).where(
            and_(AuditLog.action == "STATUS_CHANGE", AuditLog.entity_type == "asset")
        )
    )
...
async def get_predictions(
    ...
    """Predict future asset needs based on current data."""
    base_filter = Asset.deleted_at.is_(None)
    ...
    category_data = [{"category": row[0], "count": row[1], "value": float(row[2] or 0)} for row in category_result.all()]

    # Age distribution — use simple date subtraction
    today = date.today()
    ...
    # Repair frequency — total status changes
    repair_result = await db.execute(
        select(func.count(AuditLog.id)).where(
            and_(AuditLog.action == "STATUS_CHANGE", AuditLog.entity_type == "asset")
        )
    ...
    expiring_result = await db.execute(
        ...
        .where(and_(base_filter, Asset.warranty_expiry.isnot(None),
